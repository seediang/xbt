...
class TestHookInvocation:
    ...
    def test_hook_register_commands(self, reset_plugin_manager, mocker, mock_cli_group):
        ...
        manager.hook_register_commands(cli_group=mock_cli_group)

        mock_hook.assert_called_once_with(cli_group=mock_cli_group)
    ...
    def test_hook_pre_invoke_chains_modifications(self, reset_plugin_manager, mocker):
        ...
        result = manager.hook_pre_invoke(args=["original"])
    ...
    def test_hook_post_invoke_broadcasts_to_all(self, reset_plugin_manager, mocker):
        ...
        mock_result = Mock()
        manager.hook_post_invoke(args=["test"], result=mock_result)

        mock_hook.assert_called_once_with(args=["test"], result=mock_result)
